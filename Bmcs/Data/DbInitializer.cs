        ...
        public static void Initialize(BmcsContext context)
        {
            ...
            if (!context.SystemAdmins.ToList().Any())
            {
                ...
                foreach (var value in System.Enum.GetValues(typeof(SystemAdminClass)))
                {
                        ...
                        messageDetail = @"<p>本サービスを使用するためのユーザを作成します。</p>
                                            <ul class=""help-ul"">
                                                ...
                                                <li>
                                                    ...
                                                    パスワードをお忘れの際は、ログイン画面の「パスワードをお忘れの場合」からお手続きください。ご登録のメールアドレス宛に再設定用URLをお送りします。<br>
                                                </li>
                                                <li>
                                                    メールアドレス(必須)<br>
                                                    ユーザのメールアドレスを入力します。<br>
                                                    ユーザIDやパスワードを忘れた際の復旧、およびサイト管理者からのご連絡に使用します。<br>
                                                    登録がない場合、チームIDとチームパスワードによる復旧のみとなるため、必ずご登録ください。<br>
                                                </li>
                                                <li>
                                                    チーム<br>
                    ...
                    {
                        messageDetail = @"<p>ログインしているユーザの情報を変更します。</p><ul class=""help-ul""><li>ユーザID<br>ログインに使用するIDです。変更はできません。</li><li>ユーザ名(必須)<br>メッセージの投稿者として表示されます。</li><li>パスワード<br>変更する場合のみ入力してください。空のままなら変わりません。<br>入力した場合は、「確認用パスワード」に同じパスワードを入力してください。</li><li>メールアドレス<br>ユーザIDやパスワードを忘れたときの復旧と、サイト管理者からのご連絡に使用します。<br>一度登録したメールアドレスは削除できません（変更はできます）。<br>登録がない場合、復旧はチームIDとチームパスワードによる方法のみになるため、登録をおすすめします。</li><li>所属チーム<br>別のチームに移る（チームに参加する）場合は、「別のチームに移る」（所属していない場合は「チームに参加する」）を開き、チームの代表者から聞いたチームIDとチームパスワードを入力します。<br>移ると、今のチームのデータは編集できなくなります。<br>チームIDを空にして保存すると、チームから抜けます。</li></ul><p>※体験用のユーザ（サンプルチーム）は、ユーザ情報を変更できません。</p>";
                    }
                    else if ((int)value == (int)SystemAdminClass.TeamIndex)
                    {
                        messageDetail = @"<p>公開しているチームの一覧です。<br>チーム名を押すと、チームの紹介やチーム成績を確認できます。</p><ul class=""help-ul""><li>試合<br>そのチームの試合一覧を表示します。</li><li>成績<br>そのチームの成績ページを表示します。</li><li>メンバー<br>そのチームのメンバー一覧を表示します。</li><li>メッセージを送る（公開チームに所属している場合のみ表示）<br>そのチームへのダイレクトメッセージ（非公開）を送る画面を表示します。</li></ul>";
                    }
                    else if ((int)value == (int)SystemAdminClass.TeamCreate)
                    {
                        messageDetail = @"<p>チームを作成します。入力が必要なのは、チームID・チーム名・チーム略名・チームパスワード（確認用を含む）です。</p><ul class=""help-ul""><li>チームID(必須)<br>チームメイトがこのチームに参加するときに入力するIDです。<br><strong>作成後は変更できません。</strong>半角英数字がおすすめです。（既に使用されているIDはエラーとなります。）</li><li>チーム名(必須)<br>公開チーム一覧や試合結果に表示されます。</li><li>チーム略名(必須)<br>スコアボードに表示されます。10文字以内で入力します。</li><li>チームパスワード(必須)<br>チームメイトがこのチームに参加するときに入力します。ログインパスワードとは別のものです。<br>「確認用」に同じパスワードを入力してください。作成後は「チーム情報変更」で変更できます。</li><li>チームを公開する<br>公開すると、成績・試合結果・メンバー・チーム情報（代表者名・活動拠点・チーム紹介など）を<strong>誰でも（ログインしていない人も）</strong>見られるようになり、他チームとメッセージのやり取りができます。<br>非公開の場合、他チームとのメッセージのやり取りはできません。あとから変更できます。</li></ul><p>以下は任意の項目です。メールアドレス以外は、公開チームにした場合に誰でも見られます。</p><ul class=""help-ul""><li>代表者名<br>チームの代表者名を入力します。</li><li>カテゴリ・使用球<br>カテゴリ・使用球ごとに、公開チームの成績を比較できます。</li><li>活動拠点・チーム人数<br>対戦相手を探すときの参考情報になります。</li><li>メールアドレス<br>チームパスワードを忘れたときの再設定と、サイト管理者からのご連絡に使用します。<strong>他チームには表示されません。</strong><br>登録がない場合、チームパスワードを忘れたときはお問い合わせでの対応となるため、登録をおすすめします。</li><li>チーム紹介<br>チームの紹介や募集内容などを入力します。公開チーム一覧に表示されます。</li></ul>";
                    }
                    else if ((int)value == (int)SystemAdminClass.TeamEdit)
                    {
                        messageDetail = @"<p>チーム情報を変更します。<br>チームIDは画面の上部で確認できます（変更はできません）。チームメイトがチームに参加するときは、チームIDとチームパスワードを伝えてください。</p><ul class=""help-ul""><li>チーム名(必須)<br>公開チーム一覧や試合結果に表示されます。</li><li>チーム略名(必須)<br>スコアボードに表示されます。10文字以内で入力します。</li><li>チームパスワード<br>変更する場合のみ入力してください。空のままなら変わりません。<br>入力した場合は、「確認用」に同じパスワードを入力してください。</li><li>チームを公開する<br>公開すると、成績・試合結果・メンバー・チーム情報（代表者名・活動拠点・チーム紹介など）を<strong>誰でも（ログインしていない人も）</strong>見られるようになり、他チームとメッセージのやり取りができます。<br>非公開の場合、他チームとのメッセージのやり取りはできません。あとから変更できます。</li></ul><p>以下は任意の項目です。メールアドレス以外は、公開チームにした場合に誰でも見られます。</p><ul class=""help-ul""><li>代表者名<br>チームの代表者名を入力します。</li><li>カテゴリ・使用球<br>カテゴリ・使用球ごとに、公開チームの成績を比較できます。</li><li>活動拠点・チーム人数<br>対戦相手を探すときの参考情報になります。</li><li>メールアドレス<br>チームパスワードを忘れたときの再設定と、サイト管理者からのご連絡に使用します。<strong>他チームには表示されません。</strong><br>登録がない場合、チームパスワードを忘れたときはお問い合わせでの対応となるため、登録をおすすめします。</li><li>チーム紹介<br>チームの紹介や募集内容などを入力します。公開チーム一覧に表示されます。</li></ul><p>※体験用のサンプルチームは、チーム情報を変更できません。</p>";
                    }
                    ...
                    {
                        messageDetail = @"<p>チーム内の連絡や、他チームとのやり取りに使います。<br>ログインしていない場合は、公開メッセージの閲覧のみできます。</p><ul class=""help-ul""><li>表示の切り替え（ログイン時のみ表示）<br>「公開」…全チームの公開メッセージ<br>「自チームの投稿」…自チームが投稿した公開メッセージ<br>「自チームに関係」…自チームが送った・受け取ったメッセージ（公開・非公開）<br>「非公開」…チーム内のメッセージと、ダイレクトメッセージ</li><li>メッセージを投稿する<br>押すと投稿欄が開きます。タイトルとメッセージを入力して投稿します。</li><li>送信先チーム<br>空のままにすると、全チームに向けた公開メッセージになります。<br>チームを選ぶと、そのチームと自チームだけに表示されるダイレクトメッセージ（非公開）になります。<br>自チームを選ぶと、チーム内だけのメッセージになります。<br>非公開チームと体験用のユーザは、チーム内のメッセージのみ送れます。</li><li>確認＆返信<br>メッセージと返信の履歴を表示し、返信できます。</li></ul><p>送信すると、送ったメッセージの画面を表示します。メッセージは更新された順に表示されます。</p>";
                    }
                    else if ((int)value == (int)SystemAdminClass.ReplyMessage)
                    {
                        messageDetail = @"<p>メッセージと返信の履歴を表示し、返信します。返信は投稿された順に表示されます。</p><ul class=""help-ul""><li>返信<br>返信は、元のメッセージと同じ範囲に表示されます（公開メッセージへの返信は、誰でも見られます）。<br>非公開チームと体験用のユーザは、自チームの非公開メッセージにのみ返信できます。<br>ログインしていない場合は、返信欄は表示されません。</li></ul>";
                    }
