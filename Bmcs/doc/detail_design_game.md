...
| ステータス | Game.StatusClassName | Text | 試合前、試合中、確定前、確定済み |
...
2.  **ボタン制御**:
    ...
    - **試合結果編集**: ステータスが「確定前」「確定済み」の場合に表示（ロック済み EndGameLock は除く）。
    - **試合結果**: ステータスが「確定済み」（EndGame・EndGameLock）の場合に表示。
...
1.  **データ保存**:
    - 入力されたスコア、成績データを更新。
    - ステータスを「確定済み」（EndGame）に更新（確定ボタン押下時）。
2.  **再集計処理**:
