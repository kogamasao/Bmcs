...
2.  **登録処理 (`OnPostAsync`)**:
    - 入力チェック後、送信回数を制限（IP単位で60分間に5回まで。大量送信の踏み台対策）。
    - 入力値を `Inquirys` テーブルに保存。
    - `ReplyFLG` (返信済フラグ) と `CompleteFLG` (完了フラグ) は `false` で初期化。
    - **管理者へメール通知**（宛先は `EmailSettings:AdminEmail`、未設定時は `SenderEmail`）。
        - 通知内容：問い合わせID、日時、メールアドレス、ログインユーザID、タイトル、内容。
        - 本文へ埋め込むユーザ入力値は `Function/MailBody.Escape()` でHTMLエスケープし、
          件名は `Function/MailBody.EscapeSubject()` で改行を除去する（ヘッダインジェクション対策）。
        - メール送信に失敗しても問い合わせ登録は成功とし、ログ出力のみ行う。
    - 登録完了後、トップページへリダイレクト。
