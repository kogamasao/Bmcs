 ...
 *
 * 背景：打席後ランナー結果の「結果」は失策・補殺しか選べなかった。三振と同時の盗塁死（三振ゲッツー）、四球の投球での盗塁、
...
UPDATE dbo.SystemAdmin
   SET MessageDetail = REPLACE(MessageDetail, N'打者結果によって発生したプレー(補殺、失策)、及びランナーの結果を入力します。<br />', N'打者結果と同時に発生したプレー(補殺、失策、盗塁、盗塁死、WP、PB)、及びランナーの結果を入力します。<br />三振と同時の盗塁死(三振ゲッツー)、四球の投球での盗塁、振り逃げ(WP・PB)なども本エリアで入力します。ボーク・牽制死は「打席中ランナー結果」で入力します。<br />')
